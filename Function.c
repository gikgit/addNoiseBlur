...
#include "/Users/jinhe/Desktop/my-code/AddNoiseBlur/Function.h"
...
float** LoadImagePgm(char* name,int *length,int *width)
{
  ...
  fic=fopen(buff,"r");
  if (fic==NULL)
  { printf("\n- There is an error happened when open %s  -\n",buff);
    exit(-1); }
...
void blur_mask(float** h, int size_filter,int length,int width)
{
	int i,j;
	for(i=0;i<length;i++){
		for(j=0;j<width;j++){
...
void convolution(float** g, float** image, float** h, int length, int width)
{
	int i,j;
	float** tempI;
	float** temp1I;
